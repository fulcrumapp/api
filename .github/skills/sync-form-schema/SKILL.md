...
- Serialized `record_conditions` items have `linked_form_field_key`, `operator`, and either `value` or `value_field_key`; `value` may be any JSON type. Rails does not require these keys on input: missing field keys and operators serialize as null, and `value_field_key` wins when set, even if the input also supplied `value`. Unknown input keys are dropped, not rejected; do not add `additionalProperties: false`, input `required`, or an exclusive `oneOf`.
- Serialized `record_defaults` items have `source_field_key` and `destination_field_key`. Rails does not require them on input; missing keys serialize as null and unknown input keys are dropped, not rejected. Do not add `additionalProperties: false`.
